...
export default function ListeRessources(props){
    ...
    /// En attente de ListeRessourcesServer() from méthode Api ...
    useEffect(() => {
    ...
    ///////// Gestion des commandes edition et suppression
    ...
    const validEditRes = () => {
        const res = resEdit;
        SaveNewRes(res);
        ressources.some((item,i) => {
            if(item.id === res.id){
                ressources.splice(i,1,res);
                console.log("Modifications de la ressource locale : ",ressources[i]);
        ...
        {!ressources ? 
            ...
            <div className="row ligneRes" >
                <div className="col-10 libelleRes">
                    <div onClick={event => setActiveIndexRes(
                            activeIndexRes === index ? null : index
                        )}>
                    {ressource.nom}
                    </div>
                    <Collapse isOpened={activeIndexRes === index}>
                    <div className={{ show: activeIndexRes === index, hide: activeIndexRes !== index }}>
                        <ul className="ElementsRes">
                            <li>Categorie : {ressource.categorie}</li>
                            <li>Vidéo : <a href={ressource.video}>{ressource.video}</a></li>
                                <>{ressource.documents.map((doc,i)=>{
                                    return(
                                    <li>Document {i+1} : <a href={doc}>{doc}</a></li>
                                    )
                                })}
                                </>
                                <>{ressource.liens.map((lien,j) => {
                                    return(
                                    <li>Lien {j+1} : <a href={lien.url}>{lien.url}</a>
                                    <span style={{marginLeft:"15px"}}>({lien.descriptif})</span>
                                    </li>)
                                })}
                                </>
                        </ul>
